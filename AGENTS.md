...
- `source: "dona_message"`はDispatcherが生成したWorker Messaging内部通知であり、通常のSlack本文として宛先判定をやり直さず必ず処理対象とする。`worker_message_report`ではpayloadの`job_id`と`message_id`、現在の通知`event_id`を`notification_event_id`として`decide_worker_message`を必ず呼ぶ。追加のbounded文脈が必要な場合だけ`get_worker_message`へ現在の通知`event_id`を`source_event_id`として渡す。payload内の`source_event_id`は監査情報であり認可引数へ転用しない。返された`action`が`ack_internal`または`aggregate_wait`なら投稿せず完了する。`ask_user`または`report_to_user`ならSlack write直前に同じIDと現在の通知event IDで`check_worker_decision_current`を呼び、`current: true`の場合だけdecisionの`post_text`を変更せず元`reply_target`へ投稿し、投稿actionへdecisionの`post_body_sha256`を記録する。`ask_user`の投稿後はAgent Sessionを`suspended`にする。`current`がない、false、照会失敗の場合は投稿しない。decision取得失敗時は未評価reportを正常な内部受領とみなさずfail closedとする。`worker_message_silence`ではSlack writeの直前に現在の通知`event_id`とpayloadの`job_id`で`get_job_status`を読み、確認できたcurrent statusから通知要否を判断し、`worker_message_silence.current: true`の場合だけ投稿する。fieldがない、false、照会失敗の場合は古いgenerationとして投稿しない。raw本文、token、private pathを投稿しない。
