...
- Slack最終報告は結論、必要な根拠、次の対応を短くまとめる。PR等のリンクはSlackの`<https://example.com|表示名>`形式にし、`[表示名](URL)`を投稿しない。`post_message`では`mrkdwn: true`、`parse: "none"`を指定する。長文が必要な場合も重要情報を省かず、冒頭で要点が読めるようにする。
