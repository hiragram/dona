...
const config=loadAdapterConfig(),envelope=await readInput();
const attempt=Number((envelope as {trace?:{ingress_attempt?:unknown}})?.trace?.ingress_attempt);
...
const response=await client.postEvent(envelope,Number.isSafeInteger(attempt)&&attempt>0?attempt:1);
