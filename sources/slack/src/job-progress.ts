...
export class SlackJobProgressReporter {
  ...
  async deliver(input: JobProgressRequest): Promise<{ progress_id: string }> {
    ...
    catch (error) { const permanent=(error as Error & {progressPermanent?:boolean}).progressPermanent===true; throw Object.assign(error instanceof Error ? error : new Error(String(error)), { definitelyUnsent:true, progressPermanent:permanent, progressRetryable:!permanent }); }
