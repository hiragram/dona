...
const request = { schema_version: 1 as const, progress_id: "job_abc:2", workspace_id: "T123",
  channel_id: "C123", thread_ts: "1234567890.123", status: "2件中1件目: テスト中" };
...
test("reporter uses assistant thread status rather than session title", async () => {
  ...
  const reporter = new SlackJobProgressReporter(registry as never);
  assert.deepEqual(await reporter.deliver(request), { progress_id: "job_abc:2" });
  assert.deepEqual(observed, { channelId: "C123", threadTs: "1234567890.123", status: request.status });
});
