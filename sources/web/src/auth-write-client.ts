...
import { encodeAuthWriteInput, signServiceRequest, verifyServiceResponse, serviceScopeSchema, WebServiceError,
  webAuthWriteHost, webAuthWritePath, maximumServiceBodyBytes,
  type ServiceScope, type AuthWriteInput, type AuthWriteResult, type WebServiceCredential, type WebServiceCredentialLookup } from "./write-auth.js";
...
function responseLength(response: http.IncomingMessage): number {
  ...
  if (response.statusCode !== 200 || headers.get("content-type") !== "application/vnd.dona.web-auth-write-response"
...
/** Internal BFF lifecycle client. Never retry a write, including after timeout,
 * response loss or a signed denial. For logout, confirm durable revoked state
 * through the read service before clearing the browser cookie. This client
 * never creates a principal, grants a role, or executes a job/approval. */
export class WebAuthWriteClient {
  ...
  async mutate(input: AuthWriteInput): Promise<AuthWriteResult> {
    try {
      const started = performance.now();
      const body = encodeAuthWriteInput(input), before = socketIdentity(this.socketPath);
      ...
      return await new Promise<AuthWriteResult>((resolve, reject) => {
        ...
        try {
          request = http.request({ socketPath: this.socketPath, path: webAuthWritePath, method: "POST", agent: false, maxHeaderSize: 2048,
            headers: { host: webAuthWriteHost, "content-type": "application/json", "content-length": String(Buffer.byteLength(body)),
