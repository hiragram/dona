...
/** A dedicated service credential authenticates this connection only. The
 * ingress context inside body still requires current IdP/session validation. */
export function signServiceRequest(body: string, scope: ServiceScope, credentialInput: WebServiceCredential, now: string): string {
  return guard(() => {
    parseSessionServiceInput(body); const at = nowMs(now); const credential = key(credentialInput, scope, at, at, true);
    const claims = requestClaimsSchema.parse({ codec_version: 1, audience: "dona.dispatcher.web-session-service",
      key_version: credential.version, ...scope, method: "POST", path: webSessionServicePath, body_digest: hash(body),
...
export function verifyServiceResponse(proof: string, requestProof: string, requestBody: string, scope: ServiceScope,
  lookup: WebServiceCredentialLookup, now: string): SessionServiceResult {
  return guard(() => {
    ...
    if (response.result.status === "succeeded" && (response.result.principal.instance_id !== scope.instance_id
      || response.result.principal.tenant_id !== scope.tenant_id || at >= nowMs(response.result.principal.expires_at)
      || at < nowMs(response.result.principal.authenticated_at))) throw new WebServiceError();
