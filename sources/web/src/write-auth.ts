...
/** Authenticate a fixed lifecycle request. Its proof determines a one-shot
 * transaction ID on the server; the client must never retry an uncertain write. */
export function signServiceRequest(body: string, scope: ServiceScope, credentialInput: WebServiceCredential, now: string): string {
  return guard(() => {
    validateWriteInputScope(parseAuthWriteInput(body), scope); const at = nowMs(now); const credential = key(credentialInput, scope, at, at, true);
    const claims = requestClaimsSchema.parse({ codec_version: 1, audience: "dona.dispatcher.web-auth-write",
      key_version: credential.version, ...scope, method: "POST", path: webAuthWritePath, body_digest: hash(body),
...
export function verifyServiceResponse(proof: string, requestProof: string, requestBody: string, scope: ServiceScope,
  lookup: WebServiceCredentialLookup, now: string): AuthWriteResult {
  return guard(() => {
    ...
    validateWriteBinding(parseAuthWriteInput(requestBody), response.result, scope, requestProof, now);
