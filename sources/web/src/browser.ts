...
export function parseBrowserCookies(headers: RawHeaders): Readonly<{ session?: string; login?: string }> {
  ...
  for (const line of lines) {
    ...
    for (const part of line.split(";")) {
      ...
      if (name !== sessionCookieName && name !== loginCookieName) continue;
...
export function setBrowserCookie(kind: "session" | "login", value: string, maximumAgeSeconds: number): string {
  ...
  return `${kind === "session" ? sessionCookieName : loginCookieName}=${value}; Path=/; Secure; HttpOnly; SameSite=${kind === "session" ? "Strict" : "Lax"}; Max-Age=${maximumAgeSeconds}`;
}
/** Emit only after a durable revoke and its verified read-back. */
export function clearBrowserCookie(kind: "session" | "login"): string {
  return `${kind === "session" ? sessionCookieName : loginCookieName}=; Path=/; Secure; HttpOnly; SameSite=${kind === "session" ? "Strict" : "Lax"}; Max-Age=0`;
}
