...
import type { WebPrincipal } from "./domain.js";
import { encodeSessionServiceInput, signServiceRequest, verifyServiceResponse, serviceScopeSchema, WebServiceError,
  ...
  type ServiceScope, type SessionServiceInput, type WebServiceCredential, type WebServiceCredentialLookup } from "./service-auth.js";
...
export class WebSessionClient {
  ...
  async confirm(input: SessionServiceInput, identityInput: ContextIdentity): Promise<WebPrincipal | null> {
    try {
          ...
          request = http.request({ socketPath: this.socketPath, path: webSessionServicePath, method: "POST", agent: false, maxHeaderSize: 2048,
            ...
            response.once("end", () => {
              try {
                ...
                if (result.status === "denied") { finish(null); return; }
