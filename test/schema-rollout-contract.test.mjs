...
test("schema activation names the exact buildable compatibility bridge", () => {
  assert.deepEqual(rollout, {
    ...
    previous_release_sha: "ec522107117098d2250f8147ce57f4c7f48da801",
  ...
  const bridgeFromHistory = JSON.parse(execFileSync("git", ["show", `${rollout.previous_release_sha}:config/release-compatibility.json`], { encoding: "utf8" }));
  assert.deepEqual(bridgeFromHistory, previous);
});
