...
test("PR本文はcurrent baseの標準template全欄を反映して再取得検証する", async () => {
  ...
  assertContract(template, "standard PR template", [
    ...
    /新規Pull Request.*生成したtitle／本文.*local `HEAD`.*upstream.*remote head ref\/SHA.*selected base ref\/SHA.*repository default branch ref.*template取得元のbase SHA.*template blob\/hash.*snapshot/,
    /作成write直前.*current値.*同じhead\/base.*open\/closed\/merged Pull Request.*再取得/,
    /identity.*snapshot.*比較/,
    /古いdiffまたはtemplate.*title／本文でPRを作成せず.*最新identity.*生成と検証をやり直す/,
