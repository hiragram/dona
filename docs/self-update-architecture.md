...
clean stop後にpointerを切り替え、同じpaneへtargetのimmutable releaseを`-C`で指定したCodexを`dona-main`という名前で起動します。モデルとreasoning effortは`--model gpt-6-sol -c 'model_reasoning_effort="medium"'`で明示します。rollbackでこの実装の起動処理を使う場合も同じ指定になります。既存installでは先に変更を含むstable Updaterへ`--upgrade-control`し、期待SHAのversion healthを確認することが[適用条件](operations/codex-model-settings.md#既存installへの適用条件)です。Codexの更新はDonaが中央管理するため、`check_for_update_on_startup=false`をinline設定し、CLI自身の更新確認が無人起動を塞がないようにします。project-scoped `.codex/config.toml`はtrusted projectでだけ読み込まれるため、updaterが生成したexact target release pathだけをinline `projects`設定でtrustします。stdio MCPにはprotected policy由来の`config/*.env`、current manifest、updater socket、固定executableのpathだけをinline environmentとして渡し、credential値はargvやreleaseへ載せません。config rootと2つのenv fileが実directory・regular file・owner-onlyでない場合は起動前に拒否します。外部操作を禁止した起動確認promptで新しいCodex session IDを確立し、agent名、kind、pane、Codex session、foreground cwd、interactive readinessを照合してからDispatcherをstartします。その後target SHA/protocol/schema readyを確認し、Slack Adapterをstartして全workspace readyを確認します。Codex CLIの終了とproject configの仕様は[OpenAI公式のCLI reference](https://developers.openai.com/codex/cli/reference/)と[config basics](https://developers.openai.com/codex/config-basic/)に従います。
...
`dona_update`は通常のmain-agent queueから除外され、Dispatcher内の専用SQLite workerが定型日本語へrenderします。workerはSlack Adapterの認証済みinternal UDSへ、workspace/channel/thread、terminal fence、sessionの最終状態だけをtyped requestで渡します。Slack Adapterは`notification_id`を本文sectionの`block_id`へ付け、thread全pageから同じBotのexact blockをread-backしてから完了するため、投稿応答やsession status応答を失っても同じ本文を二重投稿しません。Slack投稿とAgent Session更新が完了した後だけ、そのeventのResult Envelopeを同一filesystem renameで公開し、再読後にDispatcher eventを`completed`にします。

Dispatcher/Slackの`/health/version`は`update_notification_protocol: 1`を返します。Dispatcherは通知workerが稼働し、通知DBを実際に読み書きできる場合だけreadyになります。Slack Adapterは内部reporterと0600 shared tokenを利用できる場合だけこのcapabilityを公開します。投稿応答と直後の全page read-backのどちらにもexact identity blockがない場合は、既に確定したmessage receiptを失わず通知eventを`needs_review`にします。App Manifestが受理しないcustom metadata登録を完了条件には含めません。旧runtimeでpolicy `2026-09-03.1`の`main_agent_start_failed`を補正する場合は、pointer、activation receipt、target health、target releaseの`dona-main`が一致した証拠だけを先に保存します。訂正eventは通知protocol 1のruntimeへ更新された後に新しいfenceで発行し、すでに別releaseへ進んでいれば「当時確認したtarget」と「現在の稼働SHA」を分けて報告します。
