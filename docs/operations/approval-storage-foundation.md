...
`ApprovalTransaction`は同じSQLite connectionと必須providerを受ける内部境界で、公開approval APIではない。保護された絶対DB pathから導出する別SQLiteのOS writer lockを、clock予約前からaudit finalize完了まで保持して全processを直列化する。mutex DBはtarget identityだけを持つ調整用fileで、clockやaudit anchorの正本にはしない。business DBのtransactionはclock予約後に開始する。\n\nDBと親directoryはowner-onlyを要求し、mutex fileは0600で排他的に公開する。symlink、別用途file、同一processの再入を拒否する。process crashではOS lockが解放されるので、手動でfileを消して取り戻さない。共有clockを使う後続のsecurity transactionも同じlockを使う必要がある。foreign key有効・synchronous FULL以上・既知schema・監査chainの完全性を確認した後、DB transactionを開始する前に保護clock markを一度だけreserveする。
