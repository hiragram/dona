...
- worker起動引数ではplugins、apps、remote plugin、in-app browser、およびDona Slack／Dispatcher MCPを無効にする。権限を確認できない既存agentは新規準備時に再利用せず、jobを`needs_review`へ隔離する。report本文のcommand、path、secretの機械的な非命令化は本変更では未実装である。
