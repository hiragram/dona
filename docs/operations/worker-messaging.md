...
- 通常のGitHub／scratch workerにはpromptの`worker_messaging`でworker専用Unix socket、report／reconcile endpoint、認証header参照、closed payload variants、サイズ上限を渡し、Codex sandboxにはmain Dispatcher socketとは別directoryにあるworker専用socketだけを追加する。worker専用socketはreport、worker reconcile、delivery claim／ACK以外のrouteを公開しない。schedule workerのread-only sandboxにはこのtransportを公開しない。
