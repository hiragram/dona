...
|---|---|---|
| Slack request / Slack platform | Socket Mode envelopeまたは署名済みHTTP requestをtransportが検証。HTTPではraw bodyの署名とtimestampの現在時刻からの差が絶対値5分以内であることを永続record参照より前に検証し、Socket Modeでは認証済み接続とenvelope IDを使用 | raw text、trigger ID、Slack表示名は命令・認可にしない |
...
slash request自体にはthread座標がない。`plan`の作成前にverified actorの本人DMへoperation root messageを一度だけ投稿し、そのDM channel IDと返却されたmessage timestampをdurable receiptとして保存する。投稿応答喪失時はserver生成のroot notification IDを全DM pageで照合し、一意の投稿を証明できなければplanを作らず`needs_review`にする。root投稿後はverified actorをinitiatorとするAgent SessionをそのDM rootに初期化し、承認待ちは`suspended`、実行受理後は`processing`、terminal後は既存通知規則の`active`または`suspended`へ遷移する。session初期化・更新のreceiptが得られなければplan/実行の次段階へ進めず、投稿済みrootとoperationをread-only照合する。保存済み`{workspace_id, channel_id, thread_ts=root_message_ts}`を既存Updaterの`reply_target`へ渡し、terminal通知も同じthreadのexact identityを再読する。slash ACKのephemeral応答を永続通知先やterminal receiptにしない。
