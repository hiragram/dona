...
採用: 最初のoperationは`slack.post_thread_reply.v1`とします。保存対象はworkspace、channel、thread、reply policy、mention policy、送信時のcanonical `rich_text` section（本文のliteral text要素とallowlist済みの`user` mention要素だけ）、通知用top-level fallbackの`mrkdwn: false`・`parse: "none"`・`unfurl_links: false`・`unfurl_media: false`、server-side content MAC、reconcile marker policyで、executorへは暗号化短期payload storeから復号した本文を渡します。`<!channel>`、`<!here>`、`<!everyone>`、user group mentionは拒否し、明示user mentionはsnapshotのallowlistにある最大3名だけを許可します。executorは復号した本文をliteral textとallowlist済みmention tokenに決定論的に分割し、text要素にmarkup/link/broadcast解釈をさせず、許可したuser IDだけを`rich_text`の`user`要素として構成します。top-level fallbackは通知を発火しない文字列へ正規化し、blockのuser要素と二重にmentionを載せません。分割結果の順序・user ID・表示用mention一覧とrendering policyをsemantic action hashへ結合します。実際のtyped rich_text要素列とtop-level fallbackは暗号化短期payloadに保存し、永続snapshotには安定暗号化参照、要素列とfallbackのkeyed canonical MAC、ordered mention IDを保持します。復号した要素列・fallbackを送信直前にMAC照合し、承認時のprojectionと一致しなければ送信しません。Slack Connectを含むshared channelはMVP対象外で、request作成、decision、consumeの各時点で`isShared: false`を再検証します。任意のMCP tool名、任意JSON、DM新規送信、broadcast、reaction、GitHub/Notion/Figma/Drive write、production/self-updateは対象外です。
...
採用: terminal後のrequest immutable snapshot・precondition・creation key・redacted表示projection・decision・notification/update attempt・interactive inbox・配送済みoutbox recordは90日、auditとexecution metadataは400日保持します。execution attemptが`acceptance_unknown`のまま400日境界に達した場合は、同じattemptを`needs_review`へ固定し、人間解決待ちのrequest ID・attempt ID・key version・opaque keyed execution identity・再送禁止状態だけを最小fence tombstoneへ移してtarget座標、precondition、本文由来情報を削除します。tombstoneは人間が解決するまで保持し、以後の自動marker照合・再送・fence解除は行いません。requestがterminalでもdelivery/update attemptが`acceptance_unknown`のまま90日を迎えた場合、同じattemptを`needs_review`へ固定して詳細を削除し、request ID・attempt ID・kind・key version・opaque keyed message identity・再送禁止状態だけを認証済みの最小fence tombstoneへ移します。message座標やprivate本文は残さず、後続write時は候補messageの同じkeyed identityを照合してfenceを維持します。この例外tombstoneは人間が安全に解決するまで削除せず、自動再送・fence解除をしません。その他の未完了recordは期限・復旧規則でterminalへ収束するまで削除しません。90日後はこれらの詳細をbackup expiryも含めて削除し、再配送拒否に必要なopaque keyed creation-key MAC、request ID、terminal disposition、purge時刻だけのidempotency tombstoneを400日まで保持します。tombstoneにはchannel/thread、actor、本文由来hash、precondition、private URLを残さず、各MACの`key_version`を保持し、古いsourceを再評価して新しいrequestを作りません。tombstone MAC鍵は90日以内にrotationし、旧鍵は新規MACへ使わずverification-onlyとして、その鍵を参照する最後のtombstoneの400日保持とbackup expiryがともに終了するまでOS credential storeで保護します。tombstoneとbackupを削除した後だけ旧鍵を破棄します。tombstoneのkeyが検証できない場合もfail closedにし、400日後はtombstoneを削除します。本文bindingにはapplication secretを使うHMACを用い、raw SHA-256 digestをUIや長期metadataへ残しません。承認待ちの生成本文だけはowner-onlyのapplication-level envelope encryption済みpayload storeへ保存し、鍵はDB/backupと分離したOS credential storeで管理します。requestが`rejected`、`cancelled`、`expired`、`delivery_failed`、`execution_cancelled`、`consume_expired`、`needs_review`など本文を今後利用しないterminal/invalid stateへ進む時は、同じSQLite transactionで安定payload IDのdurable deletion intentを作成して参照を無効化し、commit後に外部payloadを削除します。consume claim時はattempt専用の安定payload IDへ暗号化本文を先に複製・read-backし、同じSQLite transactionでrequest参照を無効化してattempt参照と旧payloadのdeletion intentを確定します。attemptがdurableな`succeeded` / `failed` / `needs_review`へ収束する時も同じ規則でdeletion intentを作成します。`acceptance_unknown`では最大24時間保持してreconcileし、期限後はpayloadだけを削除してattemptの`acceptance_unknown`と再送禁止fenceを維持し、自動再実行しません。状態にかかわらずattempt payloadの最大保持は24時間です。payloadはbackup対象外です。payload storeがSQLite外でも、deletion intentはpayload ID、操作、期限、状態、fenceをdurableに保持します。起動時・定期・retention処理でreconcilerが未完了intentとstore内の孤児payloadを照合し、削除後の不在read-backでだけintentを完了します。削除失敗・結果不明ではintentを保持して利用を拒否し、24時間/90日を越える保管を許容せず人間へ警告します。割当て中の遅着writeは単調fenceで拒否し、旧request IDからの再参照を禁止します。暗号化payloadにはenvelope encryption鍵version、content HMACにはMAC鍵versionをそれぞれbindingし、request/attemptがそのpayloadを参照する間は旧鍵を新規writeに使わずdecrypt/verify-onlyとしてOS credential storeに保持します。content HMACの旧鍵は最後にそのversionを参照するrequest snapshotとそのbackupが削除されるまでverification-onlyで保持します。envelope encryptionの旧鍵は最後の参照payloadの削除がread-backで確定し、backup expiryも終わってから破棄します。鍵version不明・失効・取得不能なら復号やMAC照合を成功扱いせず、executionをfail closedにします。tokenとprivate download URLは保存しません。SQLite、backup、exportはowner-onlyとします。
...
audit recordは単調sequence、直前record MAC、canonical record digest、`key_version`をMAC対象にしたhash chainとします。DB/backup外のrollback-resistant credential storeへchain ID、末尾sequence、末尾MACをanchorとして保存します。各security-relevantなrequest/decision/consume/execution/delivery/presentation状態遷移では、次anchorを直前anchor条件のCASで先にreserveし、対応するaudit rowをその状態変更と同じSQLite transactionでcommitしてから、同じtransaction IDのanchorをfinalizeします。状態だけ、またはaudit rowだけのcommitを許可しません。replay、非supervisor、cross-workspace、別actor cancelなど状態不変のsecurity rejectionも次anchorを先にCAS reserveし、拒否audit rowだけのSQLite transactionをcommitしてからanchorをfinalizeします。audit確定前に拒否応答を完了せず、予約・commit・finalizeの失敗/不明ではfail closedにします。stale/CAS競合/結果不明、未finalize reservation、chain途中の切断、sequence欠落、DB末尾とanchor不一致ではauditを完全と扱わず、reconcileまたは人間reviewまでsecurity decisionと自動実行をfail closedします。通常読取とrestoreの両方でgenesisからretention境界、末尾anchorまで検証し、retention削除時は新しいsigned genesis checkpointをanchorと同じprotocolで確定してから旧recordを削除します。

Dispatcher DBをbackupからrestoreするtransactionでは、全nonterminal/approved requestとattemptのpayload参照・HMACを検査します。backup対象外payloadが欠落または不一致なら、requestと外部call前の`claimed` attemptを`needs_review`へ固定します。`executing` attemptは送信済みの可能性があるため、payloadを削除しても`acceptance_unknown`へ先に遷移し、同じattemptのread-only marker照合を継続します。decision/claim/新規executionは拒否します。さらにDB/backup外の保護されたcredential storeへbinding generationとenforcement policy generationの単調high-water mark、各generationのcanonical digest、commit済みtransaction IDを保存します。markの欠落、読取不能、integrity不明、restore内容によるgeneration巻戻し、復元current recordのdigest/transaction ID不一致、または対応するcommit済みreservation不在では、新規requestを含むapproval経路全体をfail closedします。現在generation/digestへの二者operator再承認が完了するまで復元DBのbinding/policyをcurrentとして採用しません。payloadの再生成や元actionの自動再実行は行いません。
...
presentation update attempt:
  ...
  dispatching -> acceptance_unknown (recovery only)
  acceptance_unknown -> succeeded (exact revisionの一意なread-backのみ。人間review中もfenceを維持)
  acceptance_unknown -> needs_review (90日retention境界のみ。詳細を最小tombstoneへ移し、再送禁止fenceを維持)

execution attempt:
  not_started -> claimed -> executing -> succeeded|failed|acceptance_unknown
  claimed -> needs_review (precondition failure or restore)
  executing -> acceptance_unknown (recovery/restore/payload loss。marker照合を続け、再送しない)
...
- approval cardを含む全notification delivery attemptは`request ID + notification kind`をcreation keyとするunique constraintを持ち、retry/並行workerは状態にかかわらず既存attemptと同じmarkerへ収束します。approval delivery workerはexact draftを復号する前かつ`dispatching` fenceをcommitする同じtransactionでrequest/decision TTL、current binding/policy revision、requester authorization、supervisorのcurrent target visibility、shared状態を再取得します。不一致、期限切れ、または証明不能なら外部callなしでrequestを`expired`または`needs_review`、`pending` attemptを`aborted`へ同じtransactionで収束させ、payload deletion intentを保存して削除をreconcileします。成功した場合だけ`chat.postMessage`直前に`dispatching`とattempt fenceをdurable commitします。復旧時の`dispatching`は送信済みの可能性があるため無条件に同じattemptを`acceptance_unknown`へ移し、read-only reconcileだけを行い再送しません。
- 元threadのpending noticeはapproval cardと別のdelivery attemptとして、request ID、共通field `notification_attempt_id`、notification kind、server-side MACの一意markerへbindingします。attempt creation keyは`request ID + notification kind`としunique constraintを設け、retry/並行workerは状態にかかわらず既存attemptを返して別markerを作りません。`dispatching`をcommitする同じtransactionでcurrent request state/revisionとTTLに加え、current binding/policy revision・status・expires_at・scope digest、requesterのcurrent target membership/operation authorization、targetのshared状態、保存済みSlack executor team/app/bot identityとcredential revisionを認証済みsourceと実際の送信credentialから再取得して照合し、terminal、期限切れ、不一致、binding/policy driftまたは失効・範囲外、shared化、権限喪失、credential driftまたは証明不能ならrequestを安全側のterminalへ、notice attemptを`aborted`へ収束させて送りません。成功時だけ`chat.postMessage`直前に`dispatching` fenceをdurable commitし、timeoutまたは復旧時は`acceptance_unknown`からexact markerをread-only reconcileするだけで、0件でも再投稿しません。pagination不完全なら`acceptance_unknown`とdelivery fenceを維持し、全pageを完走するか人間が解決するまで再送・後続writeを拒否します。requestがterminalになる時点でnotice attemptが`pending`なら、同じtransactionで`aborted`へ収束させて新規noticeを送りません。
- presentation update attemptは`request ID + workspace/channel/message ID + desired presentation revision`をstable creation keyとしてunique constraintを持ちます。各desired revisionとupdate attempt ID、message座標、認証済みapp author、key versionを結ぶserver-side MACを更新後messageの専用`block_id` markerへ埋め込みます。`chat.update`では元messageのrequest ID・`notification_attempt_id`・notification kind・server-side MAC付きnotification markerを別`block_id`に必ず保持し、更新前後の同一message identityを証明します。どちらかのmarkerを欠く更新を送信しません。read-backは同じmessage座標とapp author、revision、attempt ID、MACが一意に一致した場合だけexact revisionと認め、表示本文の類似だけでは成功扱いしません。desired revisionはrequestとmessage内で単調増加し、decisionのないterminal invalidationでも同じkeyを導出できます。decision IDは存在する場合だけ監査metadataとして保持します。同じdecisionのoutbox再配送・並行worker、およびdecisionのない無効化処理の再配送は状態が`succeeded`でも既存attemptを返し、別attemptを作りません。異なるrevisionへのupdateだけを別attemptとします。credential driftなど外部call前に`aborted`となったterminal requestのupdateは、credential回復をreconcilerが認証済みread-backで確認した後、request stateを維持したまま新しいdesired revisionを一つだけtransactionalに発行し、そのrevisionの新attemptへ配送します。同一team/app/botに対する正規rotationなら、保護されたcredential registryの旧revisionから新revisionへの認証済み連鎖とrevoke範囲を検証し、新attemptにだけ後継revisionを固定します。requestのimmutable実行credential revisionは変更せず、後継revisionをdecision/consume/実行へ流用しません。principal変更・連鎖不明なら再発行せず運用reviewとfenceを維持します。旧attemptが`dispatching`または`acceptance_unknown`を一度でも経た場合はこの再発行を禁止し、message fenceを維持します。再発行時もcurrent binding、shared/visibilityと新attemptへ固定した後継credential revisionを再検証し、旧revisionを再送しません。workerは`dispatching` fence直前に保存済みpresentation revisionとcurrent desired revisionに加え、message targetのcurrent shared状態と必要なsupervisor/requester visibility、保存済みSlack executor team/app/bot identityと、そのupdate attemptへ固定したcredential revisionを認証済みsourceと実際のupdate用credentialから再取得して照合します。staleな`pending` attemptを`aborted`へ収束させ、credential drift、shared化、visibility喪失または証明不能ならAPIを呼ばずupdate attemptを`aborted`にし、認証済み運用review recordへ原因とmessage fenceを残します。requestがnonterminalなら`needs_review`へ収束させ、すでにterminalなら決定状態を維持して巻き戻しません。同じmessageに`dispatching`、`acceptance_unknown`、または90日retentionで`needs_review`へ移した再送禁止fence tombstoneがある間は後続updateを送らず、先行writeがterminalに一意確定するまで直列化します。曖昧な先行writeを飛び越えて新しい表示で上書きしません。
- request作成時にSlack executorの認証済みteam ID、app ID、bot user ID、credential revisionをrequestへimmutableに結合します。approval card・pending notice送信、presentation update、interactive decision、consume claim、`executing` fence直前には実際に各操作に使うcredentialのcurrent identity/revisionを認証済みSlack connectionから再取得し、保存済み値とすべて一致させます。ただしterminal表示更新の正規rotation復旧だけは、認証済み後継連鎖を確認して新update attemptへ固定した同一principalのrevisionと照合します。別app/botへの差し替え、revision drift、取得不能では外部callを拒否し、nonterminal requestだけを`needs_review`へ収束させます。terminal requestの表示更新ではrequest stateを維持し、update attemptを`aborted`として運用reviewへ記録します。interactive transportのapp照合だけをexecutor credentialの証明とみなしません。
- executorはconsume claim時に、信頼済み時刻を起点とする60秒の固定`execution_expires_at`とcontinuous clock readingをattemptへdurable保存し、延長しません。`claimed`から外部callへ進む直前に、この期限と経過証明、binding、policy、ordered thread revision、supervisor visibility、shared状態、Slack executor credential identity/revisionに加え、persisted requesterがcurrent targetで当該operationを要求できるmembership/authorizationを認証済みsourceから再検証します。不一致、期限切れ、continuous clockの経過証明不能は外部callなしで`needs_review`へ収束させます。成功した場合だけ`executing`とattempt fenceをdurable commitして送信します。復旧時に`executing`を観測したworkerは送信済みの可能性があるため、必ず同じattemptを`acceptance_unknown`へ移してread-only reconcileし、markerが0件でも再送しません。
...
request作成時のrequesterは、認証済みEvent Envelopeのactor、またはDispatcherが永続化したbackground job ownerからserver-sideで導出します。source event/job ID、owner kind、actor/owner ID、instance、workspaceをimmutable requestへ結合し、会話本文、LLM出力、button valueからrequester IDを受け取りません。source ownershipが欠落または一致しなければrequestを作りません。作成keyはserver-sideで`instance + workspace + source event/job ID + stable operation slot`から導出してunique constraintを設けます。同じkeyのretryでは先に保存済みrequestを取得し、そのcodec/policy revisionとcontent HMAC `key_version`のverification-only旧鍵で候補actionのcanonical内容を照合します。rotationをまたいでも一致すれば既存requestを返し、違えば状態を変えずconflictとして`needs_review`にします。旧鍵の取得・照合が不能なら同一扱いせずfail closedにし、新しいMAC鍵で再hashした差だけをaction変更と判断しません。一つのsourceで複数actionを扱う場合も、順序から変動しない明示slotをDispatcherが永続化します。creation keyとrequest placeholderを一つのDB transactionでunique claimし、`allocation_pending`、安定payload ID、lease expiry、単調fenceを保存します。勝者だけがそのfenceで暗号化payloadを割り当てます。後続workerは同じcreation key/action hashのplaceholderを再取得し、lease中は別payloadを作らず、lease失効後はfenceをCASで進めて同じpayload IDの保存状態をread-backします。未割当てを一意に証明できた場合だけ同じrequestの割当てを完了し、旧fenceの遅着writeを拒否します。存在・不在が曖昧なら新規割当てせず`needs_review`へ収束し、孤児候補は参照と外部保存結果を照合した後だけ削除します。action hashにはinstance、workspace、source ownership、typed operation、target、policy、precondition、content HMACを含めます。`encrypted_content_ref`と`rendering.encrypted_elements_ref`を含むstorage locator、暗号nonce、ciphertextなどの保存時metadataだけを除外します。

button valueにはopaque request handleとpresentation revision以外を含めません。Socket ModeにはHTTP Request Signing相当の署名付きrequestがないため、workspace別の認証済みSocket接続をprovenanceの起点とします。接続確立時に認証済みteam/app identityとworkspace registry revisionを保存し、各`interactive` / `block_actions` envelopeを、その接続identity、payloadのteam、actor user、app、container channel、message timestamp、action ID、保存済みpresentationと照合します。接続identityとpayload identityが一致しなければ拒否します。3秒以内に、envelope IDをdedup keyとするdurable interactive inboxへrequest handle、presentation revision、envelope ID、接続ID・team/app・受信時workspace registry revision、actor、container channel/message、action ID、受信時刻だけをallowlistした正規化済みproofとcommandをcommitしてからACKします。raw envelope、`message`、`blocks`、`response_url`、private draft、token、private URLはcommit前に破棄し、inbox/backupへ保存しません。ACKはapproval受理ではありません。ACK後のdecision workerは保存済みcommandだけを処理し、接続ID・受信時registry revisionをrequestの保存済みbindingとcurrent registry revisionへ照合します。revision drift、rebind、接続証明欠落はdecision拒否とauditに収束させ、duplicate envelopeは同じinbox recordへ収束させます。decision確定時はstable event IDを持つ`dona_approval` outbox rowをdecisionと同じtransactionで一度だけ作成します。decisionを伴わずrequestがresume不能な`delivery_failed`、`expired`、`needs_review`などへ初めて遷移する場合も、そのtransitionと同じtransactionで`request ID + resume slot`を一意keyとしterminal outcomeをimmutable payloadへ結合するterminal `dona_approval` outbox rowを作ります。decision前のrequest terminal eventはdecision eventと同じresume slotを競合させ、元turn/jobへ同一初回結果を二重配送しません。approval後・consume前に`execution_cancelled`、`consume_expired`、`needs_review`へ初めて失効した場合は、別の`request ID + post-decision terminal slot`を一意keyとするdurable result outboxをそのtransitionと同じtransactionで作り、persisted ownerへ最終状態を配送します。全`dona_approval` outboxにはrequest ID、単調増加するrequest event revision、stable event IDを保存します。Dispatcherはrequestごとのlast accepted revisionを元turn/jobの状態変更と同じtransactionへdurable記録し、後着の低いrevisionをstaleとして拒否します。post-decision terminal eventがapproval eventより先に届いても、その後の古いapprovalではresume stateを巻き戻しません。consume後にexecution attemptが`succeeded`、`failed`、`needs_review`へ初めて確定する場合は、同期結果でも後日のread-only reconcileでも、attempt transitionと同じtransactionで`attempt ID + execution result slot`を一意keyとする別のdurable `dona_approval` result outbox rowを作ります。persisted source ownerへredacted outcomeとattempt IDだけを配送し、approval時のone-shot eventやrequest terminal eventを実行結果の代用にしません。commit後はdurable outboxだけを配送し、process再起動時も未配送rowを回収します。`chat.update`は別のdurable attemptとして処理します。

非supervisor、別workspace、別message、古いpresentation、duplicate、期限切れもACKしてから拒否・auditします。曖昧な`chat.postMessage`結果はdelivery attempt、曖昧な`chat.update`結果は独立したpresentation update attemptの`acceptance_unknown`として保存し、どちらもblind retryしません。update attemptはrequest、message座標、desired presentation revisionへbindingし、decisionがあれば監査metadataとして結合します。creation keyを検索してから`chat.update`直前に`dispatching` fenceをdurable commitします。復旧した`dispatching`は無条件に`acceptance_unknown`へ移し、全pageを完走してread-backしたexact revisionが1件の場合だけ同じattemptを`succeeded`へ収束させます。0件またはpagination不完全は`acceptance_unknown`のまま維持し、dispatch時の相関済み決定的rejectionだけを`failed`にします。いったん`acceptance_unknown`となったupdateはexact revisionの一意なread-back時だけ`succeeded`へ進め、複数候補や人間review中もfenceを保持して`failed`へ遷移させません。0件/pagination不完全観測後もmessage-level fenceを維持して同一messageへの後続writeを送りません。

MVP replyは、承認済み本文を変えない一意なexecution attempt IDとMACをSlack Blockの`block_id`へ埋め込みます。送信前にworkspace/channel/threadの全pageで同じmarkerが0件であることを確認し、timeoutまたは`executing`復旧後は全pageを同じpagination fenceで読み、exact markerが1件かつread-backしたrich_text要素列とtop-level fallbackが保存済みcanonical MACへ一致する場合だけaccepted、markerが1件でも本文・fallback不一致またはSlackの切詰め/正規化を検出した場合は`needs_review`と再送禁止fence、0件なら`acceptance_unknown`のまま、2件以上なら`needs_review`、pagination不完全なら`acceptance_unknown`とfenceを維持します。不在観測を決定的rejectionとみなさず、別の明示操作でも同じwriteを再送しません。同文、timestamp近接、message textだけではattemptを同定しません。gatewayはSlackのtext/block上限を越える入力を拒否し、executorはsnapshotに固定した`rich_text`要素列、top-level fallbackの`mrkdwn: false`と`parse: "none"`、unfurl無効化をAPI送信前に照合します。markupやURLをlink要素へ変換せず、allowlist外のmention要素を拒否します。
...
requestが`approved`になるdecision transactionでは、まだ`pending`のpending notice attemptを同じtransactionで`aborted`へ固定し、新しい「承認待ち」通知を送らせません。そのdecision transaction、および後続の全terminal transitionでは、すでに`sent`のpending noticeすべてに現在の決定状態を示すredactedなpresentation update attemptを、単調増加するdesired revisionごとに一意に作成します。`approved`後も元threadに「承認待ち」を残しません。`approved` decisionと`rejected`、`cancelled`、`expired`、`needs_review`、`execution_cancelled`、`consume_expired`などの全terminal transitionでは、すでに`sent`のapproval cardにも現在の状態を示す操作なしのdesired revisionと一意なupdate attemptを同じtransactionで作成します。後からcard deliveryが`sent`と確定した場合も同じrevisionへ収束させ、terminal requestを`sent`へ戻しません。updateが曖昧ならmessage-level fenceを維持し、後続clickは決定済みrequestとして拒否します。`dispatching` / `acceptance_unknown`だったnoticeが後から`sent`と確定した場合もrequest stateを戻さず、同じrequest/message/desired revisionのattemptへ収束させます。全terminal transitionで同一messageの直列化とacceptance-unknown規則を適用し、古い「承認待ち」表示を放置しません。

ordered thread revisionは利用者の会話contextだけを対象にします。Dona自身が投稿したpending noticeまたはapproval cardは、認証済みapp author、request ID、共通の`notification_attempt_id`、notification kind、server-side MACがすべて一致する元notification markerを、更新後messageにも保持して識別できる場合だけ集合から除外します。presentation revision markerだけでは除外しません。本文類似やBot authorだけでは除外しません。それ以外のreply追加・削除・編集はcontext driftです。requesterのcurrent membership/authorization revisionもsnapshotへ含め、request作成、decision、consume、`executing` fence直前に再取得します。
