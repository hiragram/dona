...
audit recordは単調sequence、直前record MAC、canonical record digest、`key_version`をMAC対象にしたhash chainとします。DB/backup外のrollback-resistant credential storeへchain ID、末尾sequence、末尾MACをanchorとして保存します。各security-relevantなrequest/decision/consume/execution/delivery/presentation状態遷移では、次anchorを直前anchor条件のCASで先にreserveし、対応するaudit rowをその状態変更と同じSQLite transactionでcommitしてから、同じtransaction IDのanchorをfinalizeします。状態だけ、またはaudit rowだけのcommitを許可しません。stale/CAS競合/結果不明、未finalize reservation、chain途中の切断、sequence欠落、DB末尾とanchor不一致ではauditを完全と扱わず、reconcileまたは人間reviewまでsecurity decisionと自動実行をfail closedします。通常読取とrestoreの両方でgenesisからretention境界、末尾anchorまで検証し、retention削除時は新しいsigned genesis checkpointをanchorと同じprotocolで確定してから旧recordを削除します。
