...
- presentation update attemptは`request ID + workspace/channel/message ID + desired presentation revision`をstable creation keyとしてunique constraintを持ちます。desired revisionはrequestとmessage内で単調増加し、decisionのないterminal invalidationでも同じkeyを導出できます。decision IDは存在する場合だけ監査metadataとして保持します。同じdecisionのoutbox再配送・並行worker、およびdecisionのない無効化処理の再配送は状態が`succeeded`でも既存attemptを返し、別attemptを作りません。異なるrevisionへのupdateだけを別attemptとします。workerは`dispatching` fence直前に保存済みpresentation revisionとcurrent desired revisionに加え、message targetのcurrent shared状態と必要なsupervisor/requester visibility、保存済みSlack executor team/app/bot identityとcredential revisionを認証済みsourceと実際のupdate用credentialから再取得して照合します。staleな`pending` attemptを`aborted`へ収束させ、credential drift、shared化、visibility喪失または証明不能ならAPIを呼ばずupdate attemptを`aborted`にし、認証済み運用review recordへ原因とmessage fenceを残します。requestがnonterminalなら`needs_review`へ収束させ、すでにterminalなら決定状態を維持して巻き戻しません。同じmessageに`dispatching`、`acceptance_unknown`、または90日retentionで`needs_review`へ移した再送禁止fence tombstoneがある間は後続updateを送らず、先行writeがterminalに一意確定するまで直列化します。曖昧な先行writeを飛び越えて新しい表示で上書きしません。
