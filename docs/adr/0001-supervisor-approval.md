...
- executorはconsume claim時に、信頼済み時刻を起点とする60秒の固定`execution_expires_at`とcontinuous clock readingをattemptへdurable保存し、延長しません。`claimed`から外部callへ進む直前に、この期限と経過証明、binding、policy、ordered thread revision、supervisor visibility、shared状態に加え、persisted requesterがcurrent targetで当該operationを要求できるmembership/authorizationを認証済みsourceから再検証します。不一致、期限切れ、continuous clockの経過証明不能は外部callなしで`needs_review`へ収束させます。成功した場合だけ`executing`とattempt fenceをdurable commitして送信します。復旧時に`executing`を観測したworkerは送信済みの可能性があるため、必ず同じattemptを`acceptance_unknown`へ移してread-only reconcileし、markerが0件でも再送しません。
