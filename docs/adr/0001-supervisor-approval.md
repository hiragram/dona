...
requestが`approved`になるdecision transaction、および後続の全terminal transitionでは、すでに`sent`のpending noticeすべてに現在の決定状態を示すredactedなpresentation update attemptを、単調増加するdesired revisionごとに一意に作成します。`approved`後も元threadに「承認待ち」を残しません。`dispatching` / `acceptance_unknown`だったnoticeが後から`sent`と確定した場合もrequest stateを戻さず、同じrequest/message/desired revisionのattemptへ収束させます。全terminal transitionで同一messageの直列化とacceptance-unknown規則を適用し、古い「承認待ち」表示を放置しません。
