...
- presentation update attemptは`request ID + workspace/channel/message ID + desired presentation revision`をstable creation keyとしてunique constraintを持ちます。desired revisionはrequestとmessage内で単調増加し、decisionのないterminal invalidationでも同じkeyを導出できます。decision IDは存在する場合だけ監査metadataとして保持します。同じdecisionのoutbox再配送・並行worker、およびdecisionのない無効化処理の再配送は状態が`succeeded`でも既存attemptを返し、別attemptを作りません。異なるrevisionへのupdateだけを別attemptとします。credential driftなど外部call前に`aborted`となったterminal requestのupdateは、credential回復をreconcilerが認証済みread-backで確認した後、request stateを維持したまま新しいdesired revisionを一つだけtransactionalに発行し、そのrevisionの新attemptへ配送します。旧attemptが`dispatching`または`acceptance_unknown`を一度でも経た場合はこの再発行を禁止し、message fenceを維持します。再発行時もcurrent binding、shared/visibility、credentialを再検証し、旧revisionを再送しません。workerは`dispatching` fence直前に保存済みpresentation revisionとcurrent desired revisionに加え、message targetのcurrent shared状態と必要なsupervisor/requester visibility、保存済みSlack executor team/app/bot identityとcredential revisionを認証済みsourceと実際のupdate用credentialから再取得して照合します。staleな`pending` attemptを`aborted`へ収束させ、credential drift、shared化、visibility喪失または証明不能ならAPIを呼ばずupdate attemptを`aborted`にし、認証済み運用review recordへ原因とmessage fenceを残します。requestがnonterminalなら`needs_review`へ収束させ、すでにterminalなら決定状態を維持して巻き戻しません。同じmessageに`dispatching`、`acceptance_unknown`、または90日retentionで`needs_review`へ移した再送禁止fence tombstoneがある間は後続updateを送らず、先行writeがterminalに一意確定するまで直列化します。曖昧な先行writeを飛び越えて新しい表示で上書きしません。
- request作成時にSlack executorの認証済みteam ID、app ID、bot user ID、credential revisionをrequestへimmutableに結合します。approval card・pending notice送信、presentation update、interactive decision、consume claim、`executing` fence直前には実際に各操作に使うcredentialのcurrent identity/revisionを認証済みSlack connectionから再取得し、保存済み値とすべて一致させます。別app/botへの差し替え、revision drift、取得不能では外部callを拒否し、nonterminal requestだけを`needs_review`へ収束させます。terminal requestの表示更新ではrequest stateを維持し、update attemptを`aborted`として運用reviewへ記録します。interactive transportのapp照合だけをexecutor credentialの証明とみなしません。
