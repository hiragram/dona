...
requestがterminalになるtransactionでは、すでに`sent`のpending noticeすべてにredactedなterminal表示へのpresentation update attemptを一意に作成します。`dispatching` / `acceptance_unknown`だったnoticeが後から`sent`と確定した場合もrequest stateを戻さず、同じrequest/message/desired revisionのattemptへ収束させます。全terminal transitionで同一messageの直列化とacceptance-unknown規則を適用し、古い「承認待ち」表示を放置しません。
