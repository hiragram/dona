...
- stable Updater DBが単調増加する`update_epoch`と`request_id`、fence、旧/新release SHA、completion・control・progress・notification・provider outbox・cleanup protocol compatibility、phaseを所有する。epochはapply受理時に予約し、同一requestのrestartで再採番しない。activation失敗・rollback後も番号を再利用しない。
