...
- stable Updater DBが単調増加する`update_epoch`と`request_id`、fence、旧/新release SHA、event envelope/prompt・completion・control・progress・notification・provider outbox・cleanup protocol compatibility、phaseを所有する。epochはapply受理時に予約し、同一requestのrestartで再採番しない。activation失敗・rollback後も番号を再利用しない。
...
1. **plan/apply前:** queued/retryable_failedの未dispatch main-agent eventをdrainするか、source Event Envelope/prompt protocolをinventoryへ保存してtarget reader互換性を検証する。exact current/target SHA、inventory上の全live owner、未settle通知、provider outbox、未cleanup sandboxのprotocolに対するcompletion/progress reader・steer/cancel経路・event payload/group reader・provider payload/receipt reader・cleanup adapterと新旧runtimeのwriter互換性、schema migration能力、owner別workload inventory、通知outbox・group state、Result参照、rollback対象を永続snapshotへ結び付ける。inventoryにunknownや未解決writeがあれば適用を拒否するか対象を隔離して明示的に保護する。件数だけで安全判定しない。
...
3. **migration:** main Dispatcher DB、update notification DB、job progress DBを含む全runtime DBのonline backupをmigration前に取り、DB間watermarkとWALを含む一貫性、integrity、schema version、open-test、復元可能性を検証する。各DBのbackup/restore receiptはepoch、元SHA、schema versionと同じcutover watermarkへ束縛する。safe mode constructorでmigrationされるDBも対象とする。migrationはbackup/receiptを書いてから一度だけ実行し、失敗・応答不明時はschema/receiptを読んで判定する。新schemaを旧releaseが読書きできないならrollbackを禁止し、旧DB snapshotを復元できると証明した場合だけ旧releaseへ戻す。worker ResultはDB snapshotの外にあるため、復元後もreceipt照合を継続する。
