...
1. latest remote default branch、current branch、upstream、local diff、同じhead/baseのopen/closed Pull Requestを再取得する。
2. task fileだけを検証・明示stage・commitし、remoteへforceなしでpushする。Pull Requestがなければ、重複がないことを再確認してdefault branch向けnon-draft Pull Requestを作る。
3. local `HEAD`、upstream、Pull Request head SHAの完全一致と、base/head/state/non-draft、mergeability、base conflict、required/current CIを記録する。
4. base conflictがあればlatest baseをmergeする。各conflictを文脈ごとに解消して検証し、merge commitを通常pushする。rebase、force push、無差別な`ours`/`theirs`選択、ユーザー変更のstash・破棄は禁止する。

reviewを始める前に[review round手順](references/review-round.md)を全文読み、その監視・feedback・返信・曖昧writeの規則に従う。

## 安全境界

- review roundはpush済みのexact head SHAへ結び付ける。exact `@codex review`を1件だけ投稿し、trigger comment ID/URL、GitHub server時刻、target SHAをround recordとして保持する。
- exact triggerのreaction、trigger後のCodex-authored review・issue comment・inline comment、Pull Request head、CIを30〜60秒間隔で確認する。空reactionや新規commentがないことを成功とせず、古いroundや別SHAの結果を無視する。
...
- latest roundがcurrent Pull Request head SHAを対象とし、exact triggerの`+1`または対象SHAを明記したCodexのno-major-issues/no-findings completion commentでcleanと確定している。
