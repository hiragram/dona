...
原因jobのattentionをlate Result、operator reconcile、明示取消で解消した時点で別の未解決jobが残る場合は、配送済みattentionのownerを解除し、次の通知候補へ引き継ぎます。配送確認receiptが解消後に保存された場合も同じ引継ぎを行います。Slack actionに`error`または`ok: false`があるResultや、最後の確定Session状態が`suspended`でないResultは配送成功の証拠に使いません。
