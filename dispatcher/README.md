...
旧版で未解決attentionと同時に`all_terminal`をclaimしたDBを再起動した場合、未配送が確定した`queued` / `retryable_failed` eventだけをtransaction内でsupersedeし、job linkとgroup ownerを解除します。投稿受理が不明な状態は`job_attention_legacy_claims`に`needs_review`として記録し、自動的に再投稿・`active`遷移しません。`job attention-recovery <source_event_id>`でその監査状態を確認します。既送信通知の判別や自動再実行は行いません。
