...
export class ScheduleApiService {
  constructor(private readonly database: DispatcherDatabase, private readonly now: () => Date = () => new Date(),
    private readonly wakeScheduler: () => void = () => {}) {}

  private context(sourceEventId: string, requireActive = true): { event: EventRow; actor: Actor; target: Target } {
    const event = this.database.get(sourceEventId);
    if (!event || event.source !== "slack" || (requireActive && event.status !== "waiting_agent")) throw new ScheduleApiError(403, "unauthorized");
  ...
  private assertBinding(row: ScheduleView, context: ReturnType<ScheduleApiService["context"]>): void {
    ...
    const binding = this.context(bindingEventId, false);
  ...
  create(input: unknown) {
    ...
    try { const row = this.database.scheduler.withCodecs(codecs).create(scheduleId, built.input, occurrence.occurrence_at, context.actor, at, hash(value.idempotency_key)); this.wakeScheduler(); return { schema_version: 1, duplicate: false, schedule: this.project(this.database.scheduler.getAuthorized(row.schedule_id, context.actor)!) }; }
  ...
  list(sourceEventId: string, limit: number, cursor?: string) { const c = this.context(sourceEventId); const decoded = cursor === undefined ? 0 : Number(cursor); if (!Number.isSafeInteger(decoded) || decoded < 0 || (cursor !== undefined && String(decoded) !== cursor)) throw new ScheduleApiError(400, "invalid_cursor"); const candidates = this.database.scheduler.listAuthorized(c.actor, limit, decoded); const rows = candidates.filter(item => { try { this.assertBinding(item.schedule, c); return true; } catch (error) { if (error instanceof ScheduleApiError && error.code === "unauthorized") return false; throw error; } }); const last = candidates.at(-1); return { schema_version: 1, schedules: rows.map(item => this.project(item.schedule)), next_cursor: candidates.length === limit && last ? String(last.sequence) : null }; }
  update(scheduleId: string, input: unknown) { const value = updateInput.parse(input); const c = this.context(value.source_event_id); const current = this.database.scheduler.getAuthorized(scheduleId, c.actor); if (!current) throw new ScheduleApiError(404, "schedule_not_found"); this.assertBinding(current, c); if (current.revision !== value.expected_revision) throw new ScheduleApiError(409, "revision_conflict"); const built = this.built(value.definition, c, scheduleId, value.expected_revision + 1); const at = second(this.now().toISOString()); if (built.parsed.recurrence.kind === "once") validateCreation(built.parsed, new FakeClock(at)); const occurrence = nextPersistedOccurrence(built.parsed, at); if (!occurrence) throw new ScheduleApiError(400, "no_occurrence_in_horizon"); try { const row = this.database.scheduler.withCodecs(codecs).update(scheduleId, value.expected_revision, built.input, occurrence.occurrence_at, c.actor, at); this.wakeScheduler(); return { schema_version: 1, schedule: this.project(this.database.scheduler.getAuthorized(row.schedule_id, c.actor)!) }; } catch (error) { throw this.map(error); } }
  transition(scheduleId: string, operation: "pause"|"resume"|"cancel", input: unknown) { const value = transitionInput.parse(input); const c = this.context(value.source_event_id); const current = this.database.scheduler.getAuthorized(scheduleId, c.actor); if (!current) throw new ScheduleApiError(404, "schedule_not_found"); this.assertBinding(current, c); const desired = operation === "pause" ? "paused" : operation === "resume" ? "active" : "cancelled"; const exactRetry = current.revision === value.expected_revision + 1 && this.database.scheduler.hasAuditOperation(scheduleId, current.revision, operation, value.source_event_id); const expiryRetry = operation === "pause" && ["expired", "completed"].includes(current.state) && current.revision === value.expected_revision && this.database.scheduler.hasAuditOperation(scheduleId, current.revision, "expire", value.source_event_id); if (exactRetry || expiryRetry || (current.state === desired && current.revision === value.expected_revision)) return { schema_version: 1, duplicate: true, materialized_runs_affected: 0, schedule: this.project(current) }; const before = this.database.scheduler.pendingMaterializedCount(scheduleId); try { const row = this.database.scheduler.transition(scheduleId, value.expected_revision, operation, c.actor, second(this.now().toISOString())); const after = this.database.scheduler.pendingMaterializedCount(scheduleId); this.wakeScheduler(); return { schema_version: 1, duplicate: false, materialized_runs_affected: before - after, schedule: this.project(this.database.scheduler.getAuthorized(row.schedule_id, c.actor)!) }; } catch (error) { throw this.map(error); } }
  history(scheduleId: string, sourceEventId: string, limit: number, cursor?: string) { const c = this.context(sourceEventId); const current = this.database.scheduler.getAuthorized(scheduleId, c.actor); if (!current) throw new ScheduleApiError(404, "schedule_not_found"); this.assertBinding(current, c); let decoded: { scheduled_for: string; run_id: string } | undefined; if (cursor) { const match = /^(\d{4}-\d\d-\d\dT\d\d:\d\d:\d\dZ)\|(run_[0-9a-f-]{36})$/.exec(cursor); if (!match || !Number.isFinite(Date.parse(match[1]!)) || second(match[1]!) !== match[1]) throw new ScheduleApiError(400, "invalid_cursor"); decoded = { scheduled_for: match[1]!, run_id: match[2]! }; } const rows = this.database.scheduler.runHistory(scheduleId, c.actor, limit, decoded); const last = rows.at(-1); return { schema_version: 1, runs: rows.map(row => ({ revision:row.revision, scheduled_for:row.scheduled_for, reason:row.reason, created_at:row.created_at, started_at:row.started_at, terminal_at:row.terminal_at, status: row.status === "materialized" ? "scheduled" : row.status === "skipped" && row.reason === "misfire" ? "misfired" : row.status })), next_cursor: rows.length === limit && last ? `${last.scheduled_for}|${last.run_id}` : null }; }
  ...
  private map(error: unknown): ScheduleApiError {
    ...
    const known = new Set(["schedule_not_found", "revision_conflict", "authorization_revision_conflict", "unauthorized", "quota_exceeded",
      "invalid_creation", "invalid_transition", "invalid_next_due", "reconcile_required", "invalid_authorization", "authorization_expired", "invalid_action",
