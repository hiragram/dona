...
- 長時間 job は期限前に専用 UDS の `POST /v1/job-result-publish/renew` で現在の grant を更新する。旧 grant は元の期限まで有効で、新しい capability は private な応答で worker に返す。同じ旧 capability からの renewal 再送は同じ successor と期限を返すため、応答喪失後も回収できる。期限切れ後の再発行は自動で行わず、Dispatcher の worker 世代確認が必要になる。
