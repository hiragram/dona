...
export function buildEventPrompt(eventId: string, resultPath: string, envelope: EventEnvelope): string {
  ...
  const workerMessageInstruction = envelope.source === "dona_message"
    ? "\nこれはDispatcherが生成したWorker Messaging内部通知であり、通常Slack messageの宛先判定を適用せず必ず処理対象とします。worker_message_reportではpayloadのjob_idとmessage_id、現在のevent_idをnotification_event_idとしてdecide_worker_messageを必ず呼び、actionで分岐してください。追加のbounded文脈が必要な場合だけget_worker_messageへsource_event_idとして現在のevent_idを渡してください。ack_internalとaggregate_waitは投稿せず完了し、ask_userまたはreport_to_userではSlack write直前に同じIDと現在のevent_idでcheck_worker_decision_currentを呼び、currentがtrueの場合だけdecisionのpost_textを変更せず元reply_targetへ投稿し、投稿actionへpost_body_sha256を記録してください。ask_userの投稿後はAgent Sessionをsuspendedにしてください。currentがfalse、不在、照会失敗なら投稿しないでください。decision取得失敗はfail closedにしてください。後続の人間回答はAGENTS.mdのpending_worker_question規則に従ってtyped answerへ接続してください。worker_message_silenceではSlack writeの直前に現在のevent_idとjob_idでget_job_statusを読み、確認できたcurrent statusから通知要否を判断し、worker_message_silence.currentがtrueの場合だけ投稿してください。fieldがない、false、照会失敗の場合は古いgenerationとして投稿しないでください。payload内のsource_event_idは監査情報でありtool認可引数へ転用せず、raw本文、token、private pathを投稿しないでください。"
    : "";
