...
export class WebJobReadBroker {
  ...
  execute(input:WebJobReadInput):WebJobReadResult {
    ...
    if(input.operation==="list"){
      ...
      return finish({status:"succeeded",kind:"list",items:page.rows.map(row=>this.project(row)),next_cursor:page.next_cursor},"succeeded","none");
    }
    ...
    if(input.operation==="detail"&&match[2]===undefined){const snapshot=this.database.webJobSnapshot(identity,jobId,at);
      ...
      return finish({status:"succeeded",kind:"detail",job:this.project(snapshot.row),event_cursor:snapshot.event_cursor},"succeeded","none");}
    if(input.operation==="events"&&match[2]==="/events"&&input.cursor){const changes=this.database.listWebJobChanges(identity,jobId,input.cursor,50,at);
      ...
      return finish({status:"succeeded",kind:"events",job:this.project(current),event_cursor:changes.next_cursor,
  ...
  private project(row:JobRow):WebJobProjection {const progress=this.progress?.get(row.job_id);let result:WebJobProjection["result"]=null;
    ...
    const candidate=safe(row.last_error_code,64),error=candidate&&/^[a-z0-9_]+$/u.test(candidate)?candidate:null;return webJobProjectionSchema.parse({job_id:row.job_id,status:row.status,created_at:row.created_at,updated_at:row.updated_at,
      ...
      control:{can_cancel:["queued","preparing","dispatching","retryable_failed","running","blocked","needs_review"].includes(row.status)}});}
}
