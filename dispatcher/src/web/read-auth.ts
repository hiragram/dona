...
/** MAC validation permits only the fixed read operation. The endpoint reads
 * an audit-verified snapshot; no result grants browser or action authority. */
...
export function signServiceResponse(requestProof: string, requestBody: string, resultInput: AuthReadResult,
  ...
  return guard(() => {
    ...
    validateReadBinding(parseAuthReadInput(requestBody), result, scope);
    const response = responseClaimsSchema.parse({ codec_version: 1, audience: "dona.bff.web-auth-read-response",
