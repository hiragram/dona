...
import { serviceScopeSchema, verifyServiceRequest, signServiceResponse, parseAuthReadInput, WebServiceError,
  webAuthReadHost, webAuthReadPath, maximumServiceBodyBytes, type ServiceScope, type WebServiceCredentialLookup,
  type AuthReadResult } from "./read-auth.js";
...
function requestHeaders(request: http.IncomingMessage): { proof: string; length: number } {
  ...
  if (request.method !== "POST" || request.url !== webAuthReadPath || request.httpVersion !== "1.1"
    || headers.get("host") !== webAuthReadHost || headers.get("content-type") !== "application/json"
...
/** Dedicated credential-authenticated UDS for three fixed read operations.
 * It exposes no mutation, registry enrollment, or action capability. No production listener is started by importing this module. Runtime
 * must establish protected providers/readiness before constructing the service. */
export class WebAuthReadService {
  ...
  private async handle(request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
    ...
    try {
      ...
      const input = parseAuthReadInput(raw);
      if (performance.now() - started >= this.deadlineMs || request.socket.destroyed || response.destroyed) throw new WebServiceError();
      this.assertEndpoint();
      let reply: AuthReadResult;
      ...
      this.assertEndpoint();
      response.writeHead(200, { "content-type": "application/vnd.dona.web-auth-read-response", "content-length": String(Buffer.byteLength(proof)), connection: "close" });
      response.end(proof);
