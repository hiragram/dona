...
export class WebInternalGateway extends WebInternalService {
  constructor(socketPath: string, scope: ServiceScope, repository: WebAuthRepository,
    credentials: WebServiceCredentialLookup, now: () => string, deadlineMs = 5000, commands?: WebCommandBroker) {
    super(socketPath, scope, repository, credentials, now, deadlineMs, "all", commands);
  }
}
