...
export function migrateEventRouting(db: Database.Database, hook: () => void = () => {}): void {
  ...
  db.transaction(() => {
    ...
    const completions = db.prepare(`SELECT j.job_id, j.source_event_id, b.owner_json, b.destination_json,
      e.event_id AS notification_event_id, e.event_type, e.subject_json AS notification_subject,
      e.payload_json AS notification_payload, e.reply_target_json AS notification_destination,
      e.status AS notification_status, e.result_json AS notification_result, e.created_at AS notification_created_at
      FROM events e JOIN jobs j ON json_extract(e.subject_json, '$.job_id') = j.job_id
      JOIN job_bindings b ON b.job_id = j.job_id
      WHERE e.source = 'dona_job'
      ORDER BY e.sequence`)
      .all() as Array<Pick<JobRow, "job_id" | "source_event_id"> & { owner_json: string; destination_json: string;
        notification_event_id: string; event_type: string; notification_subject: string; notification_payload: string;
        notification_destination: string | null; notification_status: string;
    ...
    for (const row of completions) {
      ...
      const jobStatus = row.event_type.startsWith("job_") ? row.event_type.slice(4) : "";
      const payload = JSON.parse(row.notification_payload) as Record<string, unknown>;
      if (!jobStatus || payload.job_status !== jobStatus || subject.job_id !== row.job_id ||
      ...
      db.prepare(`INSERT OR IGNORE INTO job_completions
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(row.job_id, row.source_event_id, row.owner_json, row.destination_json,
          payload.result === undefined ? null : stableStringify(payload.result), jobStatus,
          row.notification_created_at, notificationState, row.notification_event_id, row.notification_result);
    }
