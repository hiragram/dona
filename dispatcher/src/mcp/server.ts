...
export function createDispatcherMcpServer(client: DispatcherJobClient, logger: Logger): McpServer {
  ...
  server.registerTool("get_job_status", {
    ...
    description: "list_thread_jobsで対象を確認します。別threadで利用者がexact job_idを明示した場合は、同一workspace/channelと依頼意図を確認して現在のsource_event_idで状態・結果・receiptを取得できます。既存receiptの再読はread-onlyですが、include_live_sessionはbounded Herdr queryと監査receipt追記を行います。曖昧応答はreceiptと永続状態で照合し、blind retryしません。",
  ...
  server.registerTool("steer_job", {
    ...
    description: "list_thread_jobsで対象を確認します。別threadで利用者がexact job_idを明示した場合は、get_job_statusで同一workspace/channelと依頼意図を確認して対象確定後だけ、現在のsource_event_idでsteerします。複数候補で対象不明なら質問し、broadcastしません。timeoutはblind retryせずget_job_statusのreceiptでread-only reconcileします。",
  ...
  server.registerTool("cancel_job", {
    ...
    description: "list_thread_jobsで対象を確認します。別threadで利用者がexact job_idを明示した場合は、get_job_statusで同一workspace/channelと依頼意図を確認して対象確定後だけ、現在のsource_event_idでcancelします。複数候補で対象不明なら質問し、成功済siblingをrollbackしません。timeoutはblind retryせずget_job_statusでread-only reconcileします。",
