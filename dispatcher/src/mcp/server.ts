...
export function createDispatcherMcpServer(client: DispatcherJobClient, logger: Logger): McpServer {
  ...
  server.registerTool("list_schedules", { title: "List schedules", description: "所有するscheduleをbounded paginationで列挙します。", inputSchema: { source_event_id: eventId, limit: z.number().int().min(1).max(100).default(50), cursor: z.string().regex(/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\dZ\|sch_[a-f0-9]{32}$/).optional() }, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async ({source_event_id, limit, cursor}) => { try { return success(await client.listSchedules(source_event_id, limit, cursor)); } catch (e) { return failure(e, logger, "list_schedules"); } });
