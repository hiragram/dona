...
describe("DispatcherApi", () => {
  test("live session opt-inをthreadへbindしdurable receiptを再読する",async()=>{
    ...
    assert.equal(denied.status,200);assert.deepEqual(calls,["get"]);
    ...
    assert.equal(live.status,200);assert.equal((live.body.live_session as {identity_match:boolean}).identity_match,true);assert.deepEqual(calls,["get","get"]);
    ...
    assert.equal(reread.status,200);assert.deepEqual(reread.body.live_session,live.body.live_session);assert.deepEqual(calls,["get","get"]);
  ...
  test("creates and reads a durable background job over UDS", async () => {
    ...
    assert.equal((await request(config.socketPath,"GET",`/v1/jobs/${job.job_id}?source_event_id=${otherChannel.body.event_id}`)).status,403);
    const otherWorkspaceEnvelope=eventEnvelope("Ev-job-api-other-workspace");
    otherWorkspaceEnvelope.subject.workspace_id="T_OTHER";otherWorkspaceEnvelope.reply_target!.workspace_id="T_OTHER";
    const otherWorkspace=await request(config.socketPath,"POST","/v1/events",otherWorkspaceEnvelope);
    assert.equal((await request(config.socketPath,"GET",`/v1/jobs/${job.job_id}?source_event_id=${otherWorkspace.body.event_id}`)).status,403);
