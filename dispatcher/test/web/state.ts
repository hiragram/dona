...
import {encodeWebAuthState,encodeWebPayload,verifyWebPayload} from "../../src/web/model.js";
...
test("login消費はpayloadを削除し、再openと別connectionでも一回だけ成功する",t=>{
 const f=setup(t);f.store.initialize("init");f.store.createLogin("create",wire.login,wire.login_payload);
...
test("sessionは消費receiptと全保持subject keyと単一principalへ結び再使用を拒否する",t=>{
 ...
 f.store.createLogin("login",wire.login,wire.login_payload);f.store.consumeLogin("consume","login",loginCookie);
...
test("署名済みrootと実metadataの旧値差替え・削除・cross-scopeを拒否する",t=>{
 ...
 f.seedRegistry();f.store.createLogin("login",wire.login,wire.login_payload);const current=encodeWebAuthState(f.readState()).canonical;
...
test("ciphertextの改変と欠落をmetadataの代用にせず予約前に拒否する",t=>{
 for(const missing of [false,true]){
  const f=setup(t);f.store.initialize("init");f.store.createLogin("login",wire.login,wire.login_payload);
...
test("auditの予約失敗と確定応答喪失を成功に変換せず再送もしない",t=>{
 for(const fault of ["reserve_before","reserve_after","finalize_before","finalize_after"] as const){
  ...
  assert.throws(()=>f.store.createLogin("create",wire.login,wire.login_payload));
...
test("session作成receiptは10秒境界で失効し古いloginからsessionを再作成しない",t=>{
 const f=setup(t);f.store.initialize("init");f.seedRegistry();f.store.createLogin("login",wire.login,wire.login_payload);f.store.consumeLogin("consume","login",loginCookie);
...
test("pure lifecycleはclock巻戻りと世代overflowを拒否しprincipalとaliasを保持する",t=>{
 ...
 const expired=pruneExpiredWebState(revoked,"2026-09-19T00:30:01.000Z");assert.deepEqual(expired.sessions,[]);assert.deepEqual(expired.aliases,state.aliases);
...
test("payload insertが失敗したらmetadataとclock参照とauditをまとめてrollbackする",t=>{
 ...
 assert.throws(()=>f.store.createLogin("failed_insert",wire.login,wire.login_payload));
