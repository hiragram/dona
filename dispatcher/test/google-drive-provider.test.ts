...
test("同じdrain内のfolder加入後に無関係parentへ移動したら隔離する", async (t) => {
...
test("page continuationとmembershipを同時commitし再起動後の無関係parentを隔離する", async (t) => {
  const db=fixture(t);
...
test("静的prior membershipの無関係parent snapshotを隔離する", async (t) => {
...
test("内容が未知の新規subfolder加入を子孫reconciliationへ隔離する",async(t)=>{
  ...
  await assert.rejects(drainDriveChanges(db,binding,{list:async()=>({changes:[
    {fileId:"subfolder",changeType:"file",time:"2026-09-07T00:00:00Z",file:{id:"subfolder",mimeType:"application/vnd.google-apps.folder",parents:["folder-1"]}}],newStartPageToken:"next-1"})},allowlist),/operation_pending/);
